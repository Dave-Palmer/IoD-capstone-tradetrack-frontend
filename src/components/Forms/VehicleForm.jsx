...
export default function VehicleFormDialog(props) {
  ...
  const handleRespone = (response) => {
    handleClose();
    let message;
    if (response.data && props.new) {
      message = `successfully added ${make} ${model}`;
    } else if (response.data && props.edit) {
      message = `Successfully updated ${make} ${model}`;
    } else {
      message = response;
    }
    ...
    <div>
      ...
      <Dialog open={props.openForm || open} onClose={handleClose}>
        ...
        <DialogContent>
          <Box
            component="form"
            onSubmit={props.new ? handleSubmitNew : handleSubmitUpdate}>
            ...
            {image.preview && (
              <Box
                ...
                <Button onClick={() => setImage({ preview: "", data: "" })}>
